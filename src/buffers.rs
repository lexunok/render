...
pub fn create(aspect_ratio: f32, device: &Device, time:f32) ->  (Vec<Buffer>, Vec<Buffer>) {
    ...
    verices.push(ring.0);
    verices.push(glow_ring_outer.0);
    verices.push(glow_ring_inner.0);

    indices.push(glow_ring_outer.1);
