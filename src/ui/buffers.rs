...
pub fn create_vertex(aspect_ratio: f32, device: &Device, time:f32) ->  Vec<Buffer> {
    ...
    let static_ring = vertex_generator::generate_static_glow_ring(aspect_ratio, 0.45, 0.4, PURPLE, BLACK);
    ...
    vec![
        ...
        get_vertex_buffer(static_ring, device),
        ]
}
...
pub fn animate_partial_ring(device: &Device, time:f32) -> (Buffer, u32) {
    let partial_ring = index_generator::generate_partial_ring(time);
